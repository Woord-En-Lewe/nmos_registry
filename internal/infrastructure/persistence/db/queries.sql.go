...
INSERT INTO devices (
    ...
    ?1, ?2, ?3, ?4, ?5, ?6,
    COALESCE(?7, 'null'), ?8, COALESCE(?9, 'null'),
    COALESCE(?10, 'null'), COALESCE(?11, 'null')
)
...
INSERT INTO flows (
    ...
    ?1, ?2, ?3, ?4, ?5, ?6,
    ?7, COALESCE(?8, 'null'), ?9, ?10,
    COALESCE(?11, 'null'), ?12, COALESCE(?13, 'null'),
    COALESCE(?14, 'null'), ?15, ?16,
    ?17, ?18, COALESCE(?19, 'null'),
    ?20
)
ON CONFLICT(id) DO UPDATE SET
...
type UpsertFlowParams struct {
	...
	BitDepth               sql.NullInt64  `json:"bit_depth"`
	DidSdid                interface{}    `json:"did_sdid"`
	GrainRate              interface{}    `json:"grain_rate"`
	FrameWidth             sql.NullInt64  `json:"frame_width"`
	FrameHeight            sql.NullInt64  `json:"frame_height"`
	InterlaceMode          sql.NullString `json:"interlace_mode"`
	Colorspace             sql.NullString `json:"colorspace"`
...
INSERT INTO receivers (
    ...
    ?1, ?2, ?3, ?4, ?5, ?6,
    COALESCE(?7, 'null'), ?8, ?9, COALESCE(?10, 'null'),
    COALESCE(?11, 'null'), ?12, ?13
)
ON CONFLICT(id) DO UPDATE SET
...
type UpsertReceiverParams struct {
	...
	InterfaceBindings    interface{}    `json:"interface_bindings"`
	SubscriptionSenderID sql.NullString `json:"subscription_sender_id"`
	SubscriptionActive   sql.NullBool   `json:"subscription_active"`
}
